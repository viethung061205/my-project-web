...
const BookingConfirm = () => {
  ...
  // ===== VNPay Confirm (ĐÃ GHÉP ĐÚNG THEO CODE BẠN ĐƯA) =====
  const handleConfirm = async () => {
    try {
      const txn_ref = new Date().getTime().toString(); // mã đơn hàng duy nhất

      // BƯỚC 1: LƯU BOOKING VÀO DB
      const saveRes = await fetch(
        "http://localhost:3000/api/vnpay/save_booking",
        {
          method: "POST",
      ...
      if (!saveRes.ok) {
        throw new Error("Lỗi lưu đơn hàng");
      }

      // BƯỚC 2: TẠO LINK THANH TOÁN VNPAY
      const res = await fetch(
        "http://localhost:3000/api/vnpay/create_payment_url",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            amount: total,
            orderInfo: `Thanh toan ve: ${customerInfo.hoten}`,
            txnRef: txn_ref
          })
        }
      );
      ...
      <div className="confirm-card">
        ...
        <div className="confirm-body">
          ...
          <section className="ticket-section">
            <h3>
              <i className="fas fa-ticket-alt"></i> Booking Details
            </h3>
