...
const BookingCheckout = () => {
  ...
  const dateInputRef = useRef(null); // 🔴 ref cho calendar
  ...
  const formatPrice = (price) => {
    ...
    if (!price) return 0;
    return Number(price.toString().replaceAll(".", "").replace(" VND", ""));
  };

  const ticketPrice = formatPrice(discountedPrice);
        ...
        <div className="day-select">
          ...
          >
            ...
            <span className="label">
              {selectedDate ? selectedDate : "View calendar"}
            </span>
          </button>
